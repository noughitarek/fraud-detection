...
def evaluate_lr(use_hho, population_size, iterations, train_size, val_size, test_size):
    starting = time.time()
    ...
    model.fit(X, y)
    if use_hho:
        dump(model, "models/hho_lr_under_f1_"+str(f1_tests_array.mean())+".joblib")
    else:
        dump(model, "models/lr_under_f1_"+str(f1_tests_array.mean())+".joblib")
...
def fitness(hyperparameters, get_model = False):
    ...
    if get_model:
        ...
        for train_index, test_index in cv.split(X_train, y_train):
            ...
            y_train_fold, _val_fold = y_train[train_index], y_train[test_index]
            ...
            # Calculate evaluation metrics
            f1_train = f1_score(y_train_fold, y_train_pred)
            f1_test = f1_score(_val_fold, y_test_pred)
