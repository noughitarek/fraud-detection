...
def evaluate_dt(use_hho, population_size, iterations, train_size, val_size, test_size):
    starting = time.time()
    ...
    model.fit(X, y)
    if use_hho:
        dump(model, "models/hho_dt_under_f1_"+str(f1_tests_array.mean())+".joblib")
    else:
        dump(model, "models/dt_under_f1_"+str(f1_tests_array.mean())+".joblib")
...
def fitness(hyperparameters, get_model = False):
    ...
    if get_model:
        ...
        if np.array([f1_test.mean(), f1_train.mean()]).std()<0.01:
